...
class FooterHost extends Component {
	...
	componentDidMount() {
		fetch("http://localhost:3000/Data/about.json")
			...
			.then((data) => {
				...
				const all = data;
				this.setState((old) => {
					return { userC: (old.userC = term), list: (old.list = term.listRight), dataS: (old.dataS = all) };
				});
			})
			.catch((error) => {
				console.log(error);
	...
	backHome() {
		this.setState({ copy: false, privacy: false });
		...
		return (
			<>
				<Term type={title} full={fullscreen} down={scrollDown} hide={HostFooter.po}>
					...
					<ol>{son()}</ol>
					<div className={HostFooter.smalest}>
						<small>{titles.notice}</small>
					</div>
				</Term>
			</>
		);
	...
	dataOut(dataKey) {
		...
		function m() {
			return (
				<Term type={a?.title} hide={HostFooter.po}>
					<h2>{a?.subtitle}</h2>
				</Term>
			);
		}
