...
class Host extends React.Component {
	...
	screenSize(sizeScreen = 0) {
		return window.matchMedia("only screen and (max-width: " + sizeScreen + "px)");
	}
	slidePoint(objToSlide, moveToLeft = false, spacesobj = 0, stopChangeNumber = 0, maxEdgeSlide = 0) {
		let movefrom;
	...
	leftOne() {
		const left = new Host();
		const size = () => {
			if (left.screenSize(832).matches) left.slidePoint(".second-host-container.once", true, 10);
			else {
	...
	rightOne() {
		...
		function widthScreen() {
			if (right.screenSize(450).matches) {
				right.slidePoint(".second-host-container.once", false, 10, -870, -894);
	...
	Trip() {
		const move = this.state.trip.map((on) => {
			return <Cap key={on.id} url={on.imageOnce} definition={on.definition} price={on.price} live={on.place} styleOne={"second-host-container once"} />;
		});
		return move;
	}
	Seller() {
		const e = this.state.best.map((l) => {
			return <Cap key={l.id} url={l.imageOnce} definition={l.definition} price={l.price} live={l.place} styleOne={"second-host-container plus"} />;
		});
		return e;
	}
	fast() {
		const del = this.state.fast.map((lo) => {
			return <Fast key={lo.id} rate={lo.rate} sunC={lo.sumComment} place={lo.place} detail={lo.detail} images={lo.image} price={lo.prices} />;
		});
		return del;
	}
	secondLeft() {
	...
	secondRight() {
		...
		function b() {
			if (right.screenSize(450).matches) {
				right.slidePoint(".second-host-container.plus", false, 10, -1050, -1050);
			} else if (right.screenSize(750).matches) {
				right.slidePoint(".second-host-container.plus", false, 15, -1250, -1290);
			} else if (right.screenSize(950).matches) {
				right.slidePoint(".second-host-container.plus", false, 15, -1050, -1070);
			} else if (right.screenSize(1030).matches) {
			...
			<React.Fragment>
				...
				<div className={HostStyle.slide}>
					<div className={HostStyle.size}>
						<Choice />
						<Btn hideLeft={this.textSlideleft} hideRight={this.textSlideRight} />
					</div>
				</div>

				<Lead heroTitle="Plan a trip with help from host around the world" btnContainer="show-text" child="Second-field" content="wrap-up-section second" slideLeft={this.leftOne} rightSlide={this.rightOne}>
					<div className="inside">{this.Trip()}</div>
				</Lead>

				<Lead heroTitle="Best Seller" btnContainer="show-text" child="Second-field" slideLeft={this.secondLeft} rightSlide={this.secondRight}>
					{this.Seller()}
				</Lead>
				<Lead heroTitle="Starting in the next 6 hours" btnContainer="show-text" child="Second-field larger" slideLeft={this.leftCard} rightSlide={this.rightCard}>
					{this.fast()}
				</Lead>
				<FooterHost />
			</React.Fragment>
